...
class TransportadoraController extends Controller
{
    ...
    {
        $total = Transportadora::where('company_id', $user->company_id)->count();
        $categories_total = TransportadoraCategories::where('company_id', $user->company_id)->count();
    ...
    {
        $params = $request->all();
        $resp = TransportadoraCategories::where('company_id', $request->user()->company_id);
        if (isset($params['per_page'])) {
            $resp = $resp->paginate($params['per_page']);
        } else {
            $resp->data = $resp->get();
    ...
    public function createCategory(Request $request)
    {
        ...
        $dados['uuid'] = Str::uuid();
        $dados['company_id'] = $request->user()->company_id;
        $resp = TransportadoraCategories::create($dados);
        if (!$resp->save()) {
    ...
    {
        $resp = TransportadoraCategories::where('uuid', $uuid)->first();
